...
import (
	...
	"github.com/mknyszek/ggg/third_party/roboto"
)

func init() {
	f, err := truetype.Parse(roboto.RegularTTF)
	if err != nil {
		panic(err)
	}
	ok := ggg.RegisterTheme(&ggg.Theme{
		...
		BorderBackgroundColor: &color.RGBA{33, 33, 33, 255},
		ChartBackgroundColor:  &color.RGBA{0, 0, 0, 255},
		ForegroundColor:       &color.RGBA{255, 255, 255, 255},
		GridlineColor:         &color.RGBA{41, 41, 41, 255},
	})
	if !ok {
